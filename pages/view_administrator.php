...
if (!$administrator_id) {
    echo "<p class='text-danger'>Invalid Administrator ID.</span>";
    exit;
}
...
if (!$administrator) {
    echo "<p class='text-danger'>Administrator not found.</span>";
    exit;
}
...
// Fetch role name
$role_name = 'N/A';
if (!empty($administrator['role_access_id'])) {
    $stmt = $pdo->prepare("SELECT role FROM role_management WHERE r_id = ?");
    $stmt->execute([$administrator['role_access_id']]);
...
<div class="main-content" style="background: #fff;">
    <div class="page-header">
        ...
        <span class="page-title">View Administrator</span>
    </div>
    ...
    <div class="it_manager-info">
        ...
        <div class="info-row">
            ...
            <div class="info-value"><?= htmlspecialchars($administrator['user_id'] ?? 'N/A') ?></div>
        </div>

        <div class="info-row">
            <div class="info-label">First Name</div>
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($administrator['first_name'] ?? 'N/A') ?></div>
        </div>

        <div class="info-row">
            <div class="info-label">Last Name</div>
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($administrator['last_name'] ?? 'N/A') ?></div>
        </div>
       ...
       <div class="info-row">
        ...
        <div class="info-value" style="display: flex; justify-content: space-between; align-items: center; margin-left: 8px;">
            <?php if (!empty($administrator['profile_image'])): ?>
                <span><?= htmlspecialchars($administrator['profile_image']) ?></span>
        ...
        <div class="info-row">
            ...
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($administrator['email'] ?? 'N/A') ?></div>
        </div>
        ...
        <div class="info-row">
            ...
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($administrator['password'] ?? 'N/A') ?></div>
        </div>
    </div>
</div>
  ...
  <!-- ✅ Floating Image Card -->
    ...
    <div id="imageCard" class="image-card">
        ...
        <div class="image-card-body">
            <img src="../uploads/administrator/<?= htmlspecialchars($administrator['profile_image']) ?>" 
                 alt="Profile Image">
        </div>
    </div>
    <?php endif; ?>
