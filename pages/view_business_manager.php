...
$pageTitle = "View Business Manager";
...
if (!$business_manager_id) {
    echo "<p class='text-danger'>Invalid Business Manager ID.</span>";
    exit;
}
...
if (!$business_manager) {
    echo "<p class='text-danger'>Business Manager not found.</span>";
    exit;
}
...
// Fetch role name
$role_name = 'N/A';
if (!empty($business_manager['role_access_id'])) {
    $stmt = $pdo->prepare("SELECT role FROM role_management WHERE r_id = ?");
    $stmt->execute([$business_manager['role_access_id']]);
...
<div class="main-content" style="background: #fff;">
    <div class="page-header">
        ...
        <span class="page-title">View Business Manager</span>
    </div>
    ...
    <div class="it_manager-info">
        ...
        <div class="info-row">
            ...
            <div class="info-value"><?= htmlspecialchars($business_manager['user_id'] ?? 'N/A') ?></div>
        </div>

        <div class="info-row">
            <div class="info-label">First Name</div>
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($business_manager['first_name'] ?? 'N/A') ?></div>
        </div>

        <div class="info-row">
            <div class="info-label">Last Name</div>
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($business_manager['last_name'] ?? 'N/A') ?></div>
        </div>
       ...
       <div class="info-row">
        ...
        <div class="info-value" style="display: flex; justify-content: space-between; align-items: center; margin-left: 8px;">
            <?php if (!empty($business_manager['profile_image'])): ?>
                <span><?= htmlspecialchars($business_manager['profile_image']) ?></span>
        ...
        <div class="info-row">
            ...
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($business_manager['email'] ?? 'N/A') ?></div>
        </div>
        ...
        <div class="info-row">
            ...
            <div class="info-colon">:</div>
            <div class="info-value"><?= htmlspecialchars($business_manager['password'] ?? 'N/A') ?></div>
        </div>
    </div>
</div>
  ...
  <!-- ✅ Floating Image Card -->
    ...
    <div id="imageCard" class="image-card">
        ...
        <div class="image-card-body">
            <img src="../uploads/business_manager/<?= htmlspecialchars($business_manager['profile_image']) ?>" 
                 alt="Profile Image">
        </div>
    </div>
    <?php endif; ?>
