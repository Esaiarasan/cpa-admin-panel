$(document).ready(function () {
    ...
    // Apply column search
    $('#itManagersTable thead tr:eq(1) th input').on('keyup change', function () {
        table
    ...
    $(document).on('click', '.delete-btn', function () {
        if (!confirm('Are you sure you want to delete this itManager?')) return;

        let itManagerId = $(this).data('id');
    ...
    // =============================
    // CREATE IT MANAGER PAGE VALIDATION
    // =============================
    const form = document.getElementById('it_managerForm');
    if (form) {
        ...
        if (urlParams.get('success') === '1') {
            ...
            alertify.success('IT Manager saved successfully!');
            setTimeout(() => {
                window.location.href = 'it_managers.php';
            }, 1500);
        }
    }
});
...
$(document).ready(function() {
    ...
    $('#it_managerForm').on('submit', function(e) {
        e.preventDefault();

        let formData = new FormData(this);

        $.ajax({
            url: '../api/update_it_manager.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            beforeSend: function() {
                alertify.dismissAll();
                alertify.message('Updating it_manager...');
            },
            success: function(response) {
                alertify.dismissAll();
                if (response.status === 'success') {
                    alertify.success(response.message || 'IT Manager updated successfully!');
                    setTimeout(() => {
                        window.location.href = 'it_managers.php';
                    }, 1500);
                } else {
